...
4. 此源码包混合了HAL和寄存器编程，并在这一版中彻底删除了对于ARM/Thumb2汇编的依赖
...
6. 条件编译 
    事实上，你会发现在此源码包中条件编译十分常见，这主要是为了保证代码的可裁剪性
...
version 0.9.5 (2017.11.28)
1. 彻底删除所有对于汇编的依赖，以保证良好的移植性
2. 添加了部分对于FSMC/FMC的支持，仅支持外部SRAM芯片的单字节寻址读写(不依赖`SRAM_HandleTypeDef`句柄)
3. 更新此版本后，因作者忙于NXP智能车比赛，有可能暂时停更
4. 修复部分bug  
